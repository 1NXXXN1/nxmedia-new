...
export default function FilmCard({
  ...
  mediaType,
  type,
  priority = false, // Default value for priority
  onFavoriteClick,
  isFavorite = false,
           ...
           <div className="absolute top-2 left-2 flex flex-col gap-1">
             <div className="flex items-center gap-1 bg-black/80 backdrop-blur-sm px-1.5 py-0.5 rounded text-xs">
               ...
               <span className="font-bold text-orange-400">
                 {(ratingKinopoisk === 0 || ratingKinopoisk === 0.0 || ratingKinopoisk === undefined) ? '-' : (ratingKinopoisk?.toFixed(1) === '0.0' ? '-' : ratingKinopoisk?.toFixed(1))}
               </span>
             </div>
             <div className="flex items-center gap-1 bg-black/80 backdrop-blur-sm px-1.5 py-0.5 rounded text-xs">
               <img src="/imdb-favicon.ico" alt="IMDb" className="w-3 h-3" />
               <span className="font-bold text-yellow-400">
                 {(ratingImdb === 0 || ratingImdb === 0.0 || ratingImdb === undefined) ? '-' : (ratingImdb?.toFixed(1) === '0.0' ? '-' : ratingImdb?.toFixed(1))}
               </span>
             </div>
           </div>
